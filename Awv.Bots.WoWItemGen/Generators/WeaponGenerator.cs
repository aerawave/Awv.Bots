...
namespace Awv.Bots.WoWItemGen.Generators
{
    public class WeaponGenerator : IGenerator<IWeapon>
    {
        ...
        public PredefinedGenerator<string> Name { get; set; }
        public PredefinedGenerator<string> Equip { get; set; }
        public PredefinedGenerator<string> Use { get; set; }
        public PredefinedGenerator<string> Corruption { get; set; }
        public PredefinedGenerator<string> Flavor { get; set; }

        private ItemRarity GenerateRarity(IRNG random)
        {
            ItemRarity rarity;
            do
            {
                rarity = Rarity.Generate(random);
            } while (rarity == ItemRarity.Artifact);
            return rarity;
        }
        public IWeapon Generate(IRNG random)
        {
            ...
            weapon.Rarity = GenerateRarity(random);
            ...
            if (Range.SetRange(0, 100).Generate(random) < 50)
            {
                var flavorFormat = new AutomationParser(Flavor.Generate(random)).Transpile();
                flavorFormat.ReplaceTag("weapon", generatorByWeaponType, random, weapon.Type.Definition.Name);
                weapon.Flavor = flavorFormat.Build(engine);
            }
        ...
        private long GenerateSellPrice(Weapon weapon, IRNG random)
        {
            ...
            var change = Range.SetRange(0, (int)maxVariance).Generate(random) - (int)Math.Floor((double)maxVariance / 2);
            return basePrice + (long)change;
        }

        private long GetMaxSellPriceVariance(Weapon weapon)
        {
            var disparity = (long)weapon.Rarity - (long)ItemRarity.Rare;
            var variance = ((long)weapon.Rarity * 7) * (disparity + 4) * 250;

            return (long)Math.Pow(variance, (double)weapon.ItemLevel.GetLevel() / 300);
        }
